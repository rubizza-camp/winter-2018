...
class BotHelper
  ...
  def extremum_change(val, hash, cur_m, cur_y, firstval = 0)
    if val < hash['min'] && firstval > -1
      hash['min'] = val
      hash['min_year'] = cur_y
      hash['min_month'] = cur_m
    end
    if val > hash['max']
      hash['max'] = val
      hash['max_year'] = cur_y
      hash['max_month'] = cur_m
    end
    hash
  end
end
class BreadBot
  ...
  def initialize(month = CURRENT_MONTH, year = CURRENT_YEAR)
    @h = {}
    page = BotHelper.new.build_page(month, year)
    page.each do |row|
      if row[NAME_COLUMN].to_s.length.positive? && row[PRICE_COLUMN_MINSK].is_a?(Numeric)
  ...
  def count(key, str)
    ...
    dog = BotHelper.new
    hash_extr = { 'minmonth' => min_month, 'minyear' => min_year, 'min' => min,
                  'maxmonth' => max_month, 'maxyear' => max_year, 'max' => max }
    (BEGIN_YEAR...CURRENT_YEAR).each do |cur_y|
      (1..12).each do |cur_m|
        val = get_price_by_date(key, cur_m, cur_y, str)
        firstval = val
        val = val.to_f / 10_000.0 if cur_y.to_i < 2017
        dog.extremum_change(val, hash_extr, cur_m, cur_y, firstval)
      end
      (1..CURRENT_MONTH).each do |cur_m|
        val = get_price_by_date(key, cur_m, CURRENT_YEAR, str)
        dog.extremum_change(val, hash_extr, cur_m, cur_y)
      end
    end
    hash_extr
  end
  ...
  def get_price_by_date(key, month, year, str)
    ...
    dog = BotHelper.new
    return b.hash[key] if b.hash.key? key
    return b.hash[b.find_match(dog.reg_build(str), b.hash)[0]] if b.find_match(dog.reg_build(str), b.hash).size == 1
  ...
  def run
    ...
    loop do
      ...
      if a.size.zero?
        ...
        key1, key2 = indexes
        puts "For similar price you also can afford #{dog.make_normal inv_hash[x[key1]]}
              and #{dog.make_normal inv_hash[x[key2]]}"
